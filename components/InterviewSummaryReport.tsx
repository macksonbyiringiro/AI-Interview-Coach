...
const InterviewSummaryReport: React.FC<InterviewSummaryReportProps> = ({ summary, onRestart }) => {
  if (!summary) {
    return (
      <div className="p-8 md:p-12 text-center">
        ...
        <p className="text-slate-600 dark:text-slate-300 mt-2">Could not generate your interview summary.</p>
        <button
          onClick={onRestart}
  ...
  return (
    <div className="p-8 md:p-12">
      <h2 className="text-3xl font-bold text-center text-slate-900 dark:text-white mb-4">Interview Report</h2>
      ...
      <div className="space-y-8 max-w-4xl mx-auto">
        {summary.results.map((result, index) => (
          <div key={index} className="bg-slate-100 dark:bg-slate-700/50 p-6 rounded-lg">
            <p className="font-bold text-lg text-slate-800 dark:text-slate-300 mb-4">
              {index + 1}. {summary.questions[result.questionIndex].question}
            </p>
            
            <div className="mb-4">
              <h4 className="font-semibold text-cyan-400 mb-2">Your Answer:</h4>
              <p className="text-slate-700 dark:text-slate-300 pl-4 border-l-2 border-slate-300 dark:border-slate-600 italic">"{result.answer}"</p>
            </div>

            <div>
              <h4 className="font-semibold text-amber-400 mb-2">AI Feedback:</h4>
              <p className="text-slate-700 dark:text-slate-300 pl-4 border-l-2 border-slate-300 dark:border-slate-600">{result.feedback}</p>
            </div>
          </div>
        ))}
      </div>
